...
class BidItems(models.Model):
    auctioneer = models.CharField(max_length=255, null=True)
    actioneer_phonenumber = models.CharField(max_length=20, null=True)
    name = models.CharField(max_length=50)
    slug = models.SlugField(unique_for_date='created')
    created = models.DateTimeField(auto_now_add=True)
    modified = models.DateTimeField(auto_now=True)
    image = models.ImageField(upload_to='images/%Y/%m/%d')
    minimum_price = models.DecimalField(max_digits=50, decimal_places=2)
    available = models.BooleanField(default=True)
